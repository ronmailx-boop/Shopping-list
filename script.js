...
function toggleRecurring(idx) {
    if (!db.currentId || !db.lists[db.currentId]) return;
...
function getRecurringForecastItems() {
    if (!db.currentId || !db.lists || !db.lists[db.currentId]) return [];
    ...
    try {
        Object.entries(db.lists).forEach(([lid, list]) => {
            if (lid === db.currentId || !list || !list.items) return;
    ...
    } catch(e) { console.warn('getRecurringForecastItems error:', e); }
    return result;
}
...
function showRecurringBanner() {
    ...
    } catch(e) { console.warn('showRecurringBanner error:', e); }
}
...
function openRecurringImportModal() {
    ...
    const listEl = document.getElementById('recurringImportList');
    ...
    listEl.innerHTML = items.map((item, i) => `
        <label style="display:flex;align-items:center;gap:10px;background:#ede9f8;border-radius:12px;padding:10px 12px;margin-bottom:7px;cursor:pointer;direction:rtl;">
            ...
            <div style="flex:1;">
                ...
                <div style="font-size:11px;color:#7b6ba8;margin-top:2px;">₪${(+item.price || 0).toFixed(2)} · מ: ${item._sourceListName}</div>
            </div>
        </label>`).join('');
    ...
    try {
        ...
        checks.forEach((cb, i) => {
            ...
            list.items.push({
                ...
                reminderValue: '', reminderUnit: '', nextAlertTime: null, alertDismissedAt: null,
                isGeneralNote: false,
                recurring: true,
                isForecast: true
            });
        ...
        if (count > 0) { save(); render(); showNotification('✅ ' + count + ' הוצאות קבועות נוספו כתחזית!'); }
    } catch(e) { console.error('importSelectedRecurring error:', e); closeRecurringImportModal(); }
}
// ════════════════════════════════════════════════════════════════════
...
function render() {
    ...
    if (activePage === 'lists') {
                            ...
                            if (isExpanded) {
                                ...
                                div.innerHTML = `
                                    ...
                                    <div style="margin-top:8px;padding-top:8px;border-top:1px solid #ede9f8;display:flex;align-items:center;justify-content:space-between;direction:rtl;">
                                        ...
                                        <button onclick="event.stopPropagation();toggleRecurring(${idx})" style="background:${item.recurring ? '#2ecc71' : '#ede9f8'};color:${item.recurring ? '#fff' : '#5b3fa6'};border:none;border-radius:20px;padding:5px 14px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;transition:all 0.2s;">
        ...
        showRecurringBanner(); // באנר הוצאות קבועות
