...
function initFirebaseAuth() {
    ...
    // Setup cloud button click handler
...
async function loginWithGoogle() {
    if (!window.firebaseAuth || !window.firebaseSignInWithPopup || !window.firebaseGoogleAuthProvider) {
    ...
    try {
        ...
        // Sync will happen automatically via setupFirestoreListener
    } catch (error) {
    ...
    unsubscribeSnapshot = window.firebaseOnSnapshot(userDocRef, (docSnapshot) => {
        if (docSnapshot.exists()) {
            ...
            if (cloudData && cloudData.data) {
                ...
                // Only update if cloud data is newer and we're not currently syncing
...
async function manualSync() {
    ...
    // Force a sync to cloud
    await syncToFirestore();

    showNotification('✅ סונכרן!');
}
