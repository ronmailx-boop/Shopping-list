...
// ========== Receipt/Bank Scanning with Gemini AI ==========
function updateFileLabel() {
    const fileInput = document.getElementById('receiptImage');
...
async function processReceipt() {
    ...
    if (!fileInput || !fileInput.files || !fileInput.files[0]) {
        showNotification('⚠️ נא לבחור תמונה תחילה', 'warning');
        return;
    }
    ...
    statusText.textContent = 'מעבד תמונה...';
    
    try {
        // Convert image to base64
        const base64Data = await fileToBase64(file);
        progressBar.style.width = '40%';
        statusText.textContent = 'שולח ל-AI...';
        
        // Call Gemini API
        const result = await callGeminiVision(base64Data);
        progressBar.style.width = '80%';
        statusText.textContent = 'מזהה פריטים...';
        
        // Parse and add items
        if (result && result.items && result.items.length > 0) {
            addScannedItems(result.items, result.cardName);
            progressBar.style.width = '100%';
            statusText.textContent = '✓ הושלם!';
            
            setTimeout(() => {
                closeModal('receiptScanModal');
                showNotification(`✅ נוספו ${result.items.length} פריטים לרשימה "${result.cardName}"`, 'success');
                
                // Reset
                progressDiv.classList.add('hidden');
                progressBar.style.width = '0%';
                fileInput.value = '';
                document.getElementById('fileLabel').textContent = '📁 בחר צילום מסך מהגלריה';
                document.getElementById('scanPreview').classList.add('hidden');
            }, 1000);
        } else {
            throw new Error('לא זוהו פריטים בתמונה');
        }
...
async function callGeminiVision(base64Data) {
    const prompt = `אתה עוזר חכם לזיהוי עסקאות מצילומי מסך של אשראי/בנק.

נתח את התמונה וזהה:
1. שם כרטיס האשראי/בנק (לדוגמה: "ויזה כאל", "מקס", "לאומי קארד")
2. כל העסקאות/חיובים שרואים בתמונה

החזר JSON בפורמט הזה בלבד:
{
  "cardName": "שם הכרטיס או הבנק",
  "items": [
    {"name": "שם העסק/החיוב", "price": 123.45, "category": "אחר"},
    {"name": "עסק אחר", "price": 67.89, "category": "אחר"}
  ]
}

חשוב:
- name: שם העסק או תיאור החיוב בעברית
- price: סכום בלבד כמספר (ללא ₪ או סימנים)
- category: תמיד "אחר"
- אם לא מצאת עסקאות, החזר items ריק []

דוגמה:
{
  "cardName": "ויזה כאל",
  "items": [
    {"name": "יוחננוף", "price": 234.50, "category": "אחר"},
    {"name": "פז תחנת דלק", "price": 250.00, "category": "אחר"}
  ]
}`;
...
function addScannedItems(items, cardName) {
    // Find or create list with cardName
    let targetListId = null;
    
    // Search for existing list with this name
    for (const [listId, list] of Object.entries(db.lists)) {
        if (list.name === cardName) {
            targetListId = listId;
            break;
    ...
    if (!targetListId) {
        const newId = 'L' + (Object.keys(db.lists).length + 1);
        db.lists[newId] = {
            name: cardName,
            url: '',
            budget: 0,
    ...
    const targetList = db.lists[targetListId];
    items.forEach(item => {
        const newItem = {
            name: item.name || 'פריט',
