    ...
    if (isGitHubPages) {
        ...
        window.signInWithPopup(window.firebaseAuth, window.googleProvider)
            .then((result) => {
                ...
                // setupFirestoreListener יקרא אוטומטית מ-onAuthStateChanged
            })
            .catch((error) => {
                console.error("❌ שגיאת התחברות:", error);
...
function setupFirestoreListener(user) {
    // ביטול listener קיים לפני יצירת חדש — מונע האזנה כפולה
    if (unsubscribeSnapshot) {
        unsubscribeSnapshot();
        unsubscribeSnapshot = null;
    }
    ...
    unsubscribeSnapshot = window.onSnapshot(userDocRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            // בדיקה: אם הענן ריק (אין רשימות, או רק L1 ריקה) אבל יש נתונים מקומיים, העלה לענן
            const cloudListCount = cloudData.lists ? Object.keys(cloudData.lists).length : 0;
            const cloudItemCount = cloudData.lists
                ? Object.values(cloudData.lists).reduce((s, l) => s + (l.items ? l.items.length : 0), 0)
                : 0;
            const cloudIsEmpty = cloudListCount === 0 || (cloudListCount === 1 && cloudItemCount === 0);

            const localItemCount = db.lists
                ? Object.values(db.lists).reduce((s, l) => s + (l.items ? l.items.length : 0), 0)
                : 0;
            const localHasData = (db.lists && Object.keys(db.lists).length > 1) || localItemCount > 0;
