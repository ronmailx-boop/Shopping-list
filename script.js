                            ...
                            div.innerHTML = `
                                <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;height:100%;">
                                    <div style="display:flex;align-items:center;gap:6px;flex:1;min-width:0;height:100%;">
                                        ...
                                        <input type="checkbox" ${item.checked ? 'checked' : ''} onchange="toggleItem(${idx})" class="w-7 h-7 accent-indigo-600" style="flex-shrink:0;" onclick="event.stopPropagation()">
                                        `}
                                        <div style="flex:1;min-width:0;overflow:hidden;display:flex;flex-direction:column;justify-content:center;cursor:pointer;">
                                            <span class="font-bold ${item.checked && !compactDeleteMode ? 'line-through text-gray-300' : ''}" style="font-size:15px;overflow:hidden;display:-webkit-box;-webkit-line-clamp:${item.dueDate ? 1 : 2};-webkit-box-orient:vertical;line-height:1.35;word-break:break-word;">
                                                <span class="item-number">${idx + 1}.</span> ${item.name}
                                            </span>
                                            ${item.dueDate ? `<span style="font-size:11px;color:#9ca3af;line-height:1.3;margin-top:2px;">${new Date(item.dueDate).toLocaleDateString('he-IL')}</span>` : ''}
                                        </div>
                                    </div>
                                    ${item.isGeneralNote ? '' : `<span class="font-black text-indigo-600" style="font-size:15px;flex-shrink:0;">₪${sub.toFixed(2)}</span>`}
                                </div>
