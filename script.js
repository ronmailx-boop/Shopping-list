...
function openBankConnectModal() {
    ...
    if (bpEl) bpEl.style.setProperty('-webkit-text-security', 'disc');
...
function togglePasswordVisibility(inputId, btnId) {
    ...
    if (!input) return;
    const current = input.style.getPropertyValue('-webkit-text-security') || 'disc';
    const isHidden = current === 'disc';
...
// ── Open credit card modal (reset fields + eye icon) ──
function openCreditCardModal() {
    const cpEl = document.getElementById('creditPassword');
    if (cpEl) { cpEl.value = ''; cpEl.style.setProperty('-webkit-text-security', 'disc'); }
    const cpBtn = document.getElementById('creditPasswordEye');
    if (cpBtn) {
        cpBtn.style.color = '#9ca3af';
        cpBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>';
    }
                ...
                if (data.status === 'done') {
                    if (!settled) {
                        ...
                        const accounts = (data.accounts || []).map(acc => {
                            const txns = (acc.txns || [])
                                .map(t => {
                                    // עסקאות שאושרו וטרם נקלטו: t.amount=0, הסכום האמיתי ב-originalAmount
                                    const rawAmount = t.chargedAmount || t.amount || t.originalAmount || 0;
                                    return {
                                        name:   t.description || 'עסקה',
                                        amount: Math.abs(rawAmount),
                                        price:  Math.abs(rawAmount),
                                        date:   t.date || '',
                                    };
                                })
