...
function render() {
    ...
    if (activePage === 'lists') {
        ...
        Object.keys(db.lists).forEach(id => {
            ...
            div.innerHTML = `
                <input type="checkbox" ${db.selectedInSummary.includes(id) ? 'checked' : ''} onchange="toggleSum('${id}')" class="w-7 h-7 accent-indigo-600">
...
function updateCloudIndicator(s) { document.getElementById('cloudIndicator').className = `w-2 h-2 rounded-full ${s === 'connected' ? 'bg-green-500' : 'bg-yellow-500 animate-pulse'}`; } //
