...
const onboardingSteps = [
    ...
    },
    {
        ...
        desc: "עקבו אחרי הסכום לתשלום בזמן אמת. המערכת מחשבת עבורכם כמה שילמתם וכמה נשאר.",
        image: "icon.png" 
    },
    {
        title: "סנכרון ושיתוף ענן",
        desc: "גבו את הנתונים לענן של גוגל ושתפו את הרשימות עם בני הבית בוואטסאפ בלחיצה אחת.",
        image: "icon.png"
    }
];
...
function toggleBottomBar() { document.querySelector('.bottom-bar').classList.toggle('minimized'); }
...
function openModal(id) { 
    ...
    if(id === 'inputForm') { document.getElementById('itemName').value = ''; document.getElementById('itemPrice').value = ''; setTimeout(() => document.getElementById('itemName').focus(), 150); }
    if(id === 'newListModal') { document.getElementById('newListNameInput').value = ''; document.getElementById('newListUrlInput').value = ''; setTimeout(() => document.getElementById('newListNameInput').focus(), 150); }
    if(id === 'editListNameModal') { const list = db.lists[db.currentId]; document.getElementById('editListNameInput').value = list.name; document.getElementById('editListUrlInput').value = list.url || ''; setTimeout(() => document.getElementById('editListNameInput').focus(), 150); }
}
...
function render() {
    ...
    if (activePage === 'lists') {
        ...
        Object.keys(db.lists).forEach(id => {
            ...
            const div = document.createElement('div'); div.className = "item-card"; div.dataset.id = id;
            div.innerHTML = `<div class="flex justify-between items-center mb-4"><div class="flex items-center gap-3 flex-1"><input type="checkbox" ${isSel ? 'checked' : ''} onchange="toggleSum('${id}')" class="w-7 h-7 accent-indigo-600"><div class="flex-1 text-2xl font-bold cursor-pointer" onclick="db.currentId='${id}'; showPage('lists')">${l.name}</div></div><button onclick="prepareDeleteList('${id}')" class="trash-btn"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg></button></div><div class="flex justify-end items-center"><span class="text-2xl font-black text-indigo-600">₪${lT.toFixed(2)}</span></div>`;
            container.appendChild(div);
        });
    }
...
function initSplashAndOnboarding() {
    ...
    setTimeout(() => {
        ...
        const hasSeen = localStorage.getItem('vplus_onboarding_seen_v1');
        if (!hasSeen) { onboarding.classList.remove('hidden'); onboarding.classList.add('flex'); renderOnboardingStep(); }
    }, 2500);
}
...
function finishOnboarding() { localStorage.setItem('vplus_onboarding_seen_v1', 'true'); document.getElementById('onboarding-overlay').classList.add('hidden'); }

// ========== Initializing ==========
window.addEventListener('DOMContentLoaded', () => {
    initSplashAndOnboarding();
    document.getElementById('onboarding-next').onclick = () => { if (currentOnboardingStep < onboardingSteps.length - 1) { currentOnboardingStep++; renderOnboardingStep(); } else finishOnboarding(); };
    document.getElementById('onboarding-skip').onclick = finishOnboarding;
    
    const bar = document.querySelector('.bottom-bar');
    if (bar) { bar.addEventListener('click', toggleBottomBar); bar.querySelectorAll('button, input').forEach(el => el.addEventListener('click', (e) => e.stopPropagation())); }
    
    render();
});

// (יתר הפונקציות: סנכרון גוגל, Sortable, ייבוא וכו' נשארות כפי שהיו בקוד המקור שלך)
