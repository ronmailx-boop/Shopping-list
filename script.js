    ...
    if (items.length === 1) {
        ...
        items.forEach(itemName => {
            if (itemName) {
                ...
                const listItems = db.lists[db.currentId].items;
                const firstChecked = listItems.findIndex(i => i.checked);
                if (firstChecked === -1) listItems.push(newItem);
                else listItems.splice(firstChecked, 0, newItem);
                addedCount++;
            }
        });
