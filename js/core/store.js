import { translations, CATEGORIES, DEMO_DATA, DEMO_NOTIFICATIONS, CATEGORY_KEYWORDS, categoryTranslations } from './constants.js';

// ============================================================
//  store.js  ג€”  ׳ ׳™׳”׳•׳ ׳׳¦׳‘ (State), ׳©׳׳™׳¨׳” ׳•׳˜׳¢׳™׳ ׳”
//  ׳׳™׳™׳‘׳ ׳: constants.js
//  ׳׳™׳•׳‘׳ ׳¢׳-׳™׳“׳™: services.js, importers.js, ui.js, app.js
// ============================================================
...
export let db = JSON.parse(localStorage.getItem('BUDGET_FINAL_V28')) || {
    ...
    selectedInSummary: [],
    lists: {
        'L1': { name: '׳”׳¨׳©׳™׳׳” ׳©׳׳™', url: '', budget: 0, isTemplate: false, items: [] }
    },
    history: [],
    templates: [],
...
// ============================================================
//  Setters ג€” ׳₪׳•׳ ׳§׳¦׳™׳•׳× ׳׳¢׳“׳›׳•׳ ׳׳©׳×׳ ׳™׳ ׳׳™׳•׳¦׳׳™׳
//  (ES6 modules: imports ׳”׳ read-only; ׳׳©׳×׳׳©׳™׳ ׳‘-setters)
// ============================================================
...
// ============================================================
//  save()  ג€”  ׳©׳׳™׳¨׳” ׳-localStorage + render + sync
// ============================================================
...
export function exportData() {
    ...
    if (typeof window.showNotification === 'function') window.showNotification('נ’¾ ׳”׳ ׳×׳•׳ ׳™׳ ׳™׳•׳¦׳׳• ׳‘׳”׳¦׳׳—׳”!');
...
function toggleItem(idx) {
    ...
    if (typeof window._showToast === 'function') {
        window._showToast({
            message: `${item.checked ? 'ג…' : 'ג—»ן¸'} "${item.name}" ${item.checked ? '׳¡׳•׳׳' : '׳”׳¡׳™׳׳•׳ ׳”׳•׳¡׳¨'}`,
            type: 'success',
...
function toggleDarkMode() {
    ...
    if (text) {
        text.textContent = document.body.classList.contains('dark-mode') ? '׳׳¦׳‘ ׳™׳•׳ ג˜€ן¸' : '׳׳¦׳‘ ׳׳™׳׳” נ™';
    }
}
...
function toggleCategorySorting() {
    ...
    if (typeof window.showNotification === 'function') {
        window.showNotification(categorySortEnabled ? 'ג… ׳׳™׳•׳ ׳׳₪׳™ ׳§׳˜׳’׳•׳¨׳™׳•׳× ׳׳•׳₪׳¢׳' : 'ג… ׳׳™׳•׳ ׳™׳“׳ ׳™ ׳׳•׳₪׳¢׳');
    }
    if (typeof window.render === 'function') window.render();
}
...
function confirmLanguageChange() {
    ...
    if (newLang === currentLang) {
        if (typeof window.showNotification === 'function')
            window.showNotification('ג“ ' + t('language') + ' ' + selector.options[selector.selectedIndex].text);
        return;
    }
...
function updateUILanguage() {
    ...
    if (confirmLangBtn) {
        const labels = { he: '׳׳©׳¨ ׳©׳™׳ ׳•׳™ ׳©׳₪׳”', en: 'Confirm Language Change', ru: '׀׀¾׀´ׁ‚׀²׀µׁ€׀´׀¸ׁ‚ׁ ׀˜׀·׀¼׀µ׀½׀µ׀½׀¸׀µ ׀¯׀·ׁ‹׀÷׀°', ro: 'Confirmִƒ Schimbarea Limbii' };
        confirmLangBtn.innerHTML = 'ג“ ' + (labels[currentLang] || labels.he);
    }
    ...
    if (tabBankEl2) tabBankEl2.textContent = 'נ¦ ׳₪׳™׳ ׳ ׳¡׳™';
...
function updateModalTexts() {
    ...
    // Common button texts
    const btnMap = {
        [t('add')]:     ['׳”׳•׳¡׳£','Add','׀”׀¾׀±׀°׀²׀¸ׁ‚ׁ','Adaugִƒ'],
        [t('cancel')]:  ['׳‘׳™׳˜׳•׳','Cancel','׀ׁ‚׀¼׀µ׀½׀°','Anuleazִƒ'],
        [t('save')]:    ['׳©׳׳•׳¨','Save','׀¡׀¾ׁ…ׁ€׀°׀½׀¸ׁ‚ׁ','Salveazִƒ'],
        [t('create')]:  ['׳¦׳•׳¨','Create','׀¡׀¾׀·׀´׀°ׁ‚ׁ','Creeazִƒ'],
        [t('delete')]:  ['׳׳—׳§','Delete','׀£׀´׀°׀»׀¸ׁ‚ׁ','ָ˜terge'],
        [t('complete')]:['׳”׳©׳׳','Complete','׀—׀°׀²׀µׁ€ׁˆ׀¸ׁ‚ׁ','Finalizeazִƒ'],
        [t('update')]:  ['׳¢׳“׳›׳','Update','׀׀±׀½׀¾׀²׀¸ׁ‚ׁ','Actualizeazִƒ'],
        [t('close')]:   ['׳¡׳’׳•׳¨','Close','׀—׀°׀÷ׁ€ׁ‹ׁ‚ׁ','ֳnchide'],
        [t('importBtn')]:['׳™׳™׳‘׳','Import','׀˜׀¼׀¿׀¾ׁ€ׁ‚׀¸ׁ€׀¾׀²׀°ׁ‚ׁ','Importִƒ'],
    };
...
function exitDemoMode() {
    ...
    if (Object.keys(db.lists).length === 0) {
        db.lists['L1'] = { name: '׳”׳¨׳©׳™׳׳” ׳©׳׳™', url: '', budget: 0, isTemplate: false, items: [] };
        db.currentId = 'L1';
    } else {
...
function showDemoBanner() {
    ...
    div.innerHTML = '<span style="font-size:20px;">נ¯</span><div style="flex:1;"><div style="font-size:12px;font-weight:900;color:white;">׳׳¦׳‘ ׳“׳׳• ׳₪׳¢׳™׳</div><div style="font-size:10px;color:rgba(255,255,255,0.8);">׳׳׳• ׳ ׳×׳•׳ ׳™ ׳“׳•׳’׳׳” ג€” ׳—׳§׳•׳¨ ׳‘׳—׳•׳₪׳©׳™׳•׳×!</div></div><button onclick="exitDemoMode()" style="background:rgba(255,255,255,0.25);border:1.5px solid rgba(255,255,255,0.4);color:white;font-size:10px;font-weight:800;padding:5px 14px;border-radius:99px;cursor:pointer;font-family:system-ui,sans-serif;">׳™׳¦׳™׳׳” ׳׳“׳׳•</button>';
    ...
    sheet.innerHTML = `
        <div style="display:flex;justify-content:flex-end;margin-bottom:6px;">
            <button onclick="document.getElementById('demoPromptOverlay').remove();localStorage.setItem('vplus_demo_seen','true');"
                style="background:rgba(0,0,0,0.06);border:none;border-radius:50%;width:32px;height:32px;font-size:20px;cursor:pointer;color:#888;">ֳ—</button>
        </div>
        <div style="width:40px;height:4px;background:#e5e7eb;border-radius:99px;margin:0 auto 20px;"></div>
        <div style="text-align:center;margin-bottom:20px;">
            <div style="font-size:48px;margin-bottom:12px;">נ¯</div>
            <div style="font-size:20px;font-weight:900;color:#1e1b4b;margin-bottom:6px;">׳‘׳¨׳•׳ ׳”׳‘׳ ׳-Vplus Pro!</div>
            <div style="font-size:13px;color:#6b7280;line-height:1.6;">׳¨׳•׳¦׳” ׳׳¨׳׳•׳× ׳׳™׳ ׳”׳׳₪׳׳™׳§׳¦׳™׳” ׳ ׳¨׳׳™׳×<br>׳¢׳ ׳ ׳×׳•׳ ׳™׳ ׳׳׳™׳×׳™׳™׳ ׳׳₪׳ ׳™ ׳©׳×׳×׳—׳™׳?</div>
        </div>
