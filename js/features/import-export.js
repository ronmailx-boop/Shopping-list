...
import { db, save, setActivePage, setPendingImportText, setDetectedListType, detectedListType, currentLang } from '../core/store.js';
import { detectCategory } from '../core/store.js';
// ============================================================
//  importers.js  ג€”  ׳›׳ ׳׳•׳’׳™׳§׳× ׳”׳™׳™׳‘׳•׳
//  ׳׳™׳™׳‘׳ ׳: store.js
//  ׳׳™׳•׳‘׳ ׳¢׳-׳™׳“׳™: ui.js, app.js
...
function importFromText() {
    ...
    for (let i = startIndex; i < lines.length; i++) {
        ...
        if (!line || line.includes('נ›’') || line.includes('נ’°') || line.includes('׳¡׳”"׳›') || line === '---') continue;

        let added = false;

        // Full format: ג¬/ג… *name* (xN) - ג‚×price
        const fullMatch = line.match(/[ג¬ג…]\s*\*([^*]+)\*\s*\(x(\d+)\)\s*-\s*ג‚×([\d.]+)/);
        if (fullMatch) {
            const name = fullMatch[1].trim();
            const qty  = parseInt(fullMatch[2]);
            items.push({ name, price: parseFloat(fullMatch[3]) / qty, qty, checked: line.includes('ג…'), category: detectCategory(name), cloudId: _newCloudId() });
            added = true;
        }
        if (!added) {
            const m = line.match(/^[ג€¢\-]\s*\*?([^(]+)\*?\s*\(x(\d+)\)/);
            if (m) {
    ...
    if (items.length === 0) { alert('׳׳ ׳ ׳׳¦׳׳• ׳׳•׳¦׳¨׳™׳ ׳‘׳˜׳§׳¡׳˜'); return; }
    ...
    save();
    if (typeof window.showNotification === 'function') window.showNotification(`ג… ׳™׳•׳‘׳׳• ${items.length} ׳׳•׳¦׳¨׳™׳!`);
}
...
function toggleClipboardAutoOpen() {
    ...
    if (label) { label.textContent = isOn ? '׳׳•׳₪׳¢׳' : '׳׳•׳©׳‘׳×'; label.style.color = isOn ? '#7367f0' : '#94a3b8'; }
}
...
function showClipboardImportModal(text) {
    ...
    const typeNames = { shopping: 'נ›’ ׳¨׳©׳™׳׳× ׳§׳ ׳™׳•׳×', appointment: 'נ¥ ׳×׳•׳¨/׳₪׳’׳™׳©׳”', tasks: 'ג… ׳¨׳©׳™׳׳× ׳׳©׳™׳׳•׳×', general: 'נ“ ׳¨׳©׳™׳׳” ׳›׳׳׳™׳×' };
    if (detectedTypeName) detectedTypeName.textContent = typeNames[type] || 'נ“ ׳¨׳©׳™׳׳” ׳›׳׳׳™׳×';
...
function detectListType(text) {
    ...
    const hasDateTime  = /\d{1,2}[\/\.\-]\d{1,2}[\/\.\-]?\d{0,4}/.test(text) || /\d{1,2}:\d{2}|׳‘׳©׳¢׳”|׳©׳¢׳”/.test(text);
    const hasPhone     = /0\d{1,2}[\-\s]?\d{3,4}[\-\s]?\d{3,4}|׳˜׳׳₪׳•׳|׳˜׳:|׳ ׳™׳™׳“/.test(text);
    ...
    const hasPrice        = /\d+\s*׳©"׳—|ג‚×\s*\d+|\d+\s*׳©׳§׳/.test(text);
...
function acceptClipboardImport() {
    ...
    localStorage.setItem('clipboardState', JSON.stringify(state));
    if (typeof window.showNotification === 'function') window.showNotification('ג… ׳‘׳—׳¨ ׳¨׳©׳™׳׳” ׳׳• ׳¦׳•׳¨ ׳¨׳©׳™׳׳” ׳—׳“׳©׳” ׳׳”׳•׳¡׳₪׳× ׳”׳₪׳¨׳™׳˜׳™׳');
}
...
function parseAppointmentText(text) {
    ...
    // Name ג€” "׳×׳•׳¨ ׳[׳©׳]"
    const nameMatch1 = text.match(/׳×׳•׳¨ ׳(\w+)/);
    if (nameMatch1) {
        const clinicMatch = text.match(/(׳׳›׳‘׳™׳“׳ ׳˜|׳›׳׳׳™׳×|׳׳׳•׳—׳“׳×|׳׳׳•׳׳™׳×)[\s\w-]*/);
        name = clinicMatch ? `׳×׳•׳¨ ׳${nameMatch1[1]} - ${clinicMatch[0]}` : `׳×׳•׳¨ ׳${nameMatch1[1]}`;
    }
    if (!name) {
        for (const l of lines) {
            if (l.includes('׳“"׳¨') || l.includes('׳“׳•׳§׳˜׳•׳¨') || l.includes('׳¨׳•׳₪׳') || l.includes('׳₪׳¨׳•׳₪') || l.includes('׳׳¨׳₪׳׳”') || l.includes('׳§׳׳™׳ ׳™׳§׳”')) {
                name = l; break;
            }
        }
    }
    ...
    // Doctor
    const doctorMatch = text.match(/(?:׳)?׳’׳‘['׳³]?\s+(\w+\s+\w+)|(?:׳)?׳“["׳´]׳¨\s+(\w+\s+\w+)|(?:׳)?׳₪׳¨׳•׳₪['׳³]?\s+(\w+\s+\w+)/);
    const doctorName  = doctorMatch ? 'נ‘₪ ' + doctorMatch[0] : '';

    const noteParts = [];
    if (doctorName) noteParts.push(doctorName);
    if (location)   noteParts.push('נ“ ' + location);
    if (phone)      noteParts.push('ג˜ן¸ ' + phone);
    if (url)        noteParts.push('נ”— ' + url);

    for (const l of lines) {
        if (l.length < 3) continue;
        const used = (name && l.includes(name.replace('׳×׳•׳¨ ׳','').replace(' - ','')))
            || (location && l.includes(location))
            || (phone && l.includes(phone))
            || (url && l.includes(url))
            || (doctorName && l.includes(doctorName.replace('נ‘₪ ','')))
    ...
    return [{
        name: name || '׳₪׳’׳™׳©׳”', price: 0, qty: 0, checked: false,
        category: '׳×׳•׳¨/׳₪׳’׳™׳©׳”', note: noteParts.join('\n'),
...
function parseShoppingListText(text) {
    return text.split('\n').map(l => l.trim()).filter(Boolean).map(line => {
        let price = 0;
        let name  = line;
        const m   = line.match(/(.+?)\s*[ג‚×]?\s*(\d+(?:\.\d+)?)\s*(?:׳©"׳—|׳©׳§׳)?/);
...
function startVoiceInput() {
    if (!recognition) {
        ...
        if (!recognition) {
            if (typeof window.showNotification === 'function') window.showNotification('׳”׳“׳₪׳“׳₪׳ ׳׳ ׳×׳•׳׳ ׳‘׳–׳™׳”׳•׳™ ׳§׳•׳׳™', 'error');
            return;
        }
    }
    ...
    recognition.onerror  = (e) => {
        ...
        if (typeof window.showNotification === 'function') {
            window.showNotification(e.error === 'no-speech' ? '׳׳ ׳–׳•׳”׳” ׳“׳™׳‘׳•׳¨, ׳ ׳¡׳” ׳©׳•׳‘' : '׳©׳’׳™׳׳” ׳‘׳–׳™׳”׳•׳™ ׳§׳•׳׳™', 'warning');
        }
    };
...
function parseVoiceInput(text) {
    ...
    if (items.length === 0) {
        if (typeof window.showNotification === 'function') window.showNotification('׳׳ ׳–׳•׳”׳• ׳׳•׳¦׳¨׳™׳, ׳ ׳¡׳” ׳©׳•׳‘', 'warning');
        return;
    }
    ...
    if (items.length === 1) {
        ...
        _n('itemPrice')?.focus();
        if (typeof window.showNotification === 'function') window.showNotification(`נ₪ "${itemName}" - ׳”׳•׳¡׳£ ׳׳—׳™׳¨ ׳׳• ׳׳—׳¥ ׳”׳•׳¡׳£`);
    } else {
        let count = 0;
        ...
        if (typeof window.showNotification === 'function') window.showNotification(`ג… ׳ ׳•׳¡׳₪׳• ${count} ׳׳•׳¦׳¨׳™׳: ${items.join(', ')}`);
    }
}

// ג”€ג”€ Voice Actions (bought / tobuy) ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€
...
function startVoiceAction(mode) {
    ...
    try {
        ...
        if (typeof window.showNotification === 'function') window.showNotification('נ₪ ׳׳׳•׳¨ ׳©׳ ׳׳•׳¦׳¨...', 'info');
    } catch (e) {
        _stopVoiceAction();
    }
}
...
function _handleVoiceActionResult(transcripts, mode) {
    ...
    for (const transcript of transcripts) {
        ...
        if (mode === 'bought') {
            if (!found) {
                if (typeof window.showNotification === 'function') window.showNotification(`ג“ ׳׳ ׳ ׳׳¦׳: "${transcript}"`, 'warning');
                return;
            }
            if (found.checked) {
                if (typeof window.showNotification === 'function') window.showNotification(`ג„¹ן¸ "${found.name}" ׳›׳‘׳¨ ׳׳¡׳•׳׳`, 'info');
                return;
            }
            found.checked = true;
            save();
            if (typeof window.showNotification === 'function') window.showNotification(`ג… "${found.name}" ׳¡׳•׳׳ ׳›׳ ׳¨׳›׳©`);
        } else if (mode === 'tobuy') {
            if (!found) {
            ...
            found.checked = false;
            save();
            if (typeof window.showNotification === 'function') window.showNotification(`נ›ן¸ "${found.name}" ׳”׳•׳—׳–׳¨ ׳׳¨׳©׳™׳׳”`);
        }
        return;
    }
}
...
function _showAddItemPrompt(name) {
    if (confirm(`"${name}" ׳׳ ׳ ׳׳¦׳ ׳‘׳¨׳©׳™׳׳”. ׳׳”׳•׳¡׳™׳£?`)) {
        ...
        save();
        if (typeof window.showNotification === 'function') window.showNotification(`ג• "${name}" ׳ ׳•׳¡׳£ ׳׳¨׳©׳™׳׳”`);
    }
}
...
async function performTranslation() {
    ...
    try {
        for (let i = 0; i < list.items.length; i++) {
            if (statusDiv) statusDiv.textContent = `׳׳×׳¨׳’׳ ${i + 1} ׳׳×׳•׳ ${list.items.length}...`;
        ...
        if (typeof window.showNotification === 'function') window.showNotification(`ג… ׳×׳•׳¨׳’׳׳• ${translated} ׳׳•׳¦׳¨׳™׳!`);
    } catch (e) {
        progressDiv?.classList.add('hidden');
        if (typeof window.showNotification === 'function') window.showNotification('׳©׳’׳™׳׳” ׳‘׳×׳¨׳’׳•׳', 'error');
    }
...
async function processReceipt() {
    ...
    try {
        ...
        if (result.responses?.[0]?.error) throw new Error(`׳©׳’׳™׳׳× API: ${result.responses[0].error.message}`);
        ...
        if (typeof window.showNotification === 'function') window.showNotification(error.message || '׳©׳’׳™׳׳” ׳‘׳¡׳¨׳™׳§׳× ׳”׳§׳‘׳׳”', 'error');
        progressDiv?.classList.add('hidden');
        _resetBtn();
    }
}
...
function parseReceiptText(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/׳¡׳”"׳›|׳¡׳”׳›|total|sum|׳×׳׳¨׳™׳|date|׳§׳•׳₪׳”|׳§׳‘׳׳”|receipt|׳—\.׳₪|׳—׳₪|vat|׳׳¢"׳|׳׳¢׳|׳‘׳¨׳§׳•׳“|barcode|׳×׳•׳“׳”|thank|׳©׳¢׳”|time|׳›׳×׳•׳‘׳×|address|׳˜׳׳₪׳•׳|phone|׳׳©׳¨׳׳™|credit|׳׳–׳•׳׳|cash/i.test(line)) continue;

        const m1 = line.match(/^(.+?)\s+(ג‚×|׳©"׳—|׳©׳—)?\s*([\d.,]+)\s*(ג‚×|׳©"׳—|׳©׳—)?$/);
        if (m1) {
        ...
        if (i < lines.length - 1) {
            ...
            const priceMatch = nextLine.match(/^(ג‚×|׳©"׳—|׳©׳—)?\s*([\d.,]+)\s*(ג‚×|׳©"׳—|׳©׳—)?$/);
            if (priceMatch) {
...
export { importFromText, checkClipboardOnStartup, openManualImport, showClipboardImportModal, acceptClipboardImport, dismissClipboardImport, importTextToList, detectListType, changeDetectedType, toggleClipboardAutoOpen, updateDetectedTypeFromInput, performTranslation, processReceipt, createListFromReceipt, startVoiceInput, stopVoiceInput, parseVoiceInput, startVoiceAction, initVoiceAction, initVoiceRecognition };
