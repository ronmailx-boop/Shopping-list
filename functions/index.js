...
// ─────────────────────────────────────────────
// פונקציה 1: התראה כשיש שינוי ברשימה (Firestore trigger)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// פונקציה 2: תזכורות מתוזמנות - Cron כל דקה
// ─────────────────────────────────────────────
...
exports.testNotification = functions.https.onRequest(async (req, res) => {
  try {
    ...
    await sendFCMToTokens([token], {
      title: '🧪 התראת בדיקה',
      body: 'זוהי התראת בדיקה מ-VPlus',
      data: { type: 'test' }
    });

    res.status(200).send('התראה נשלחה בהצלחה');
  } catch (error) {
